...
- 不提交 `.env`、`.env.local`、真实 API key、真实数据库密码、Langfuse secret。
- 不提交本地日志、缓存、`__pycache__`、`.pytest_cache`、`.ruff_cache`、IDE 配置。
- `.env.example` / `.env.production.example` 只保留占位符，不放真实 endpoint、账号或路径。
- CSpider 原始数据来自 [taolusi/chisp](https://github.com/taolusi/chisp)，请按其许可自行下载，项目不内置数据集。

欢迎大家提 issue / PR：包括更多数据库评测、更细的 SQL 错误归因、更好的 few-shot 选择策略、schema linking 优化、生产监控指标补充，以及部署文档改进。
